...
> **Note:** this snippet relies on `answer-game-reducer`, which is removed in
...
> `sortNumbersMachine` / `numberMatchMachine` directly via `createActor(...)`
> — see the `definition.test.ts` files for the canonical pattern. Kept here
