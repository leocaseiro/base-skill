...
{/_ NOTE: this snippet relies on answer-game-reducer, which is removed in PR 1c.
The XState-first engine replaces it (see §5 above and §9 for the
overlay invariant). For new tests, drive `wordSpellMachine` /
`sortNumbersMachine` / `numberMatchMachine` directly via
`createActor(...)` (see `definition.test.ts` files for the canonical
pattern). Kept here as a historical reference until PR 1c lands. (review #19) _/}
...
2. Confirm overlays are conditionally rendered on `engine.phase`,
   ...
   was removed in PR 1b — sounds now fire from machine `entry`
   actions and overlays gate directly on `engine.phase`.
