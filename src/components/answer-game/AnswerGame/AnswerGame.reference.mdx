...
| `useGameTTS`            | `useGameTTS.ts`            | Returns `{ speakTile, speakPrompt, speakPromptOnDemand }` — TTS surface for games             | —                                                                              |

---

## TTS Surface (`useGameTTS`)

The `useGameTTS` hook returns three callbacks with three distinct
voice-availability semantics. The user notification mechanism differs
per-callback by intent:

| Callback              | When to use                                                  | Preferred voice unavailable → behaviour                                                                   |
| --------------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `speakTile`           | Auto playback on tile interactions (drag pickup, tile reads) | Silent fallback to the OS default voice; global `VoiceUnavailableWarning` banner informs the user.        |
| `speakPrompt`         | Auto playback at round start / auto-trigger paths            | Silent return — does NOT play with the OS default; banner informs the user.                               |
| `speakPromptOnDemand` | User-initiated speak actions (audio button, tile tap)        | Opens the `VoiceUnavailableDialogProvider` AlertDialog so the user can fix the configuration immediately. |

The global `VoiceUnavailableWarning` banner (mounted in `_app.tsx`) is the
ambient notification surface. The `VoiceUnavailableDialogProvider` AlertDialog
(also mounted in `_app.tsx`) is the on-demand notification surface that interrupts
...
PR 1a adds the optional `engineDispatch` prop. When set, every reducer
dispatch — including the provider's own mount-effect `INIT_ROUND` /
`RESUME_ROUND` plus every `PLACE_TILE` / `TYPE_TILE` / etc. from
`Slot`, `NumeralTileBank`, `useKeyboardInput`, and `useTouchKeyboardInput`
— is mirrored to `engineDispatch`. XState-first games (NumberMatch in
PR 1a; WordSpell + SortNumbers in PR 1b; SpotAll in PR 1d) pass their
`useGameEngine(...).send` (wrapped in a stable callback via `useRef`)
...
The wrapped dispatch is built with `useCallback` (empty deps) and reads
the latest `engineDispatch` via a `useRef`, so its identity is stable
even when the engine's `send` is recreated on each machine state
