...
/* Red shake emphasis — during the ~300ms .animate-shake window, thicken
   the stone-outline SVG's stroke and add a red drop-shadow glow so the
   wrong-tile feedback reads loudly without the previous rectangular
   outline that was clipped to the slot box (and was applying to every
   .animate-shake element, including empty-slot reject shakes). Scoped
   to .dragon-cave-stone-outline--wrong only — empty/correct outlines
   are untouched. drop-shadow follows the SVG alpha shape so the glow
   hugs the stone silhouette. */
.skin-dragon-cave .animate-shake .dragon-cave-stone-outline--wrong path {
  stroke-width: 32;
}
.skin-dragon-cave .animate-shake .dragon-cave-stone-outline--wrong {
