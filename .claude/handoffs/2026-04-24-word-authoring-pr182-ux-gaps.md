...
2. [ ] **Apply the UX bundle** (one commit on this branch, no push until user reviews):
   ...
   - **Empty-state CTA must check shipped data, not just the current filter.** When a search returns 0 results, look up the word in the full shipped AUS index. If it exists at another level, show `"adore" exists at Level 4 — clear your level filter to see it.` instead of `✨ Make up this word?`. Otherwise keep the current CTA. Implementation: extend `filter.ts` with `loadShippedWordLevels(region): Promise<Map<string, number>>` (or replace `loadShippedIndex` with the richer map — caller does `.has()` the same way), load it on mount in `WordLibraryExplorer.tsx`, branch the empty-state render based on `shippedMap.get(searchTerm)`. Also consider checking drafts (`draftStore.listDrafts`) for the same word at another level, though shipped is the higher-value case.
   - TDD: new test `deletes a chip via the editor` in `AuthoringPanel.test.tsx`; new test `resets form state when opened with a different word` in `WordLibraryExplorer.authoring.test.tsx`; new test `empty-state shows "exists at other level" instead of CTA when word is shipped at a filtered-out level`.
